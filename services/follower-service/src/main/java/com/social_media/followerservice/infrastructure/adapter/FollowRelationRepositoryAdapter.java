...
public class FollowRelationRepositoryAdapter implements FollowRelationRepository {
    ...
    public FollowRelation save(FollowRelation followRelation) {
        FollowRelationEntity entity = mapper.toEntity(followRelation);
        FollowRelationEntity saved = jpaRepository.save(entity);
        return mapper.toDomain(saved);
    }

    @Override
    public Optional<FollowRelation> findById(Long id) {
        return jpaRepository.findById(id).map(mapper::toDomain);
    }

    @Override
    public boolean existsByFollowerIdAndFollowingId(UserId followerId, UserId followingId) {
        return jpaRepository.existsByFollowerIdAndFollowingId(followerId.value(), followingId.value());
    }

    @Override
    public Page<FollowRelation> findByFollowerId(UserId followerId, Pageable pageable) {
        return jpaRepository.findByFollowerId(followerId.value(), pageable)
                .map(mapper::toDomain);
    }

    @Override
    public Page<FollowRelation> findByFollowingId(UserId followingId, Pageable pageable) {
        return jpaRepository.findByFollowingId(followingId.value(), pageable)
                .map(mapper::toDomain);
    }

    @Override
    public void deleteByFollowerIdAndFollowingId(UserId followerId, UserId followingId) {
